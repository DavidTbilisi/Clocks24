...
**"This month's archetype" panel** mixes the Archetype Wheel's data into the simulator: it's not a static section, it tracks whatever month/year `st` currently points to. `renderArchetypePanel()` is called both from the initial `loadArchetypeData()` (which fetches `months.json`/`weekCards.json`, absent everywhere else in this file) and from the end of every `render()`, so it updates immediately as you page through months/years — same `getISOWeek()`-over-`daysInMonth` approach as `archetypes.js`, but keyed on `st.y`/`st.m` instead of the current real date/year. Unlike `archetypes.html`'s wheel, there's no competing "selected" visual state here, so the archetype name just reuses the existing `.mo-N.mo-N` season-color rules directly (no new color rules needed).
